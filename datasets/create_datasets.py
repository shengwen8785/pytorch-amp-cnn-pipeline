...
def read_imagenette_dataset(image_size:int, data_dir: str, size:str, local_rank: int):
    """
    ...
    # Use context to make sure that only the main process will read data.
    with torch_distributed_zero_first(local_rank):
        display_message(logger, local_rank, f"Downloading and loading Imagenette dataset from {data_dir}.")
        train_dataset = Imagenette(data_dir, split='train', size=size, download=True, transform=train_transforms)
        val_dataset = Imagenette(data_dir, split='val', size=size, download=True, transform=val_transforms)
